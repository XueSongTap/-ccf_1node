...
void CompSixAxis::CompUpdate(){
    ...
    // or maybe -Gx -Gz ? remain to check
    compAngleY = CompFilterProcess(compAngleY, accelAngleY, -Gz);
    compAngleZ = CompFilterProcess(compAngleZ, accelAngleZ, Gy);

}

void CompSixAxis::CompAnglesGet(float *YAngle, float *ZAngle){
...
void CompSixAxis::CompAccelCalculate(){
    ...
    accelAngleZ = atan2f(Az, sqrtf(SQRE(Ax) + SQRE(Ay)));


    accelAngleY = FormatAccelRange(accelAngleY, Ax);
    accelAngleZ = FormatAccelRange(accelAngleZ, Ax);

    
    // accelAngleZ = FormatAccelRange(accelAngleZ, Az);

    //弧度
    std::cout << "accelAngleX :" << accelAngleY * RAD_TO_DEG_RATIO<< std::endl;
    std::cout << "accelAngleY :" << accelAngleZ * RAD_TO_DEG_RATIO << std::endl;
...
//
float CompSixAxis::
FormatAccelRange(float accelAngle, float accelX){
    if(accelX< 0.0f){
        accelAngle =  PI - accelAngle;
        
    }else if (accelX > 0.0f && accelAngle < 0.0f){
